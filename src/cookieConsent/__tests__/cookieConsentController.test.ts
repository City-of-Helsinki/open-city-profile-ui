...
import mockWindowLocation, {
  MockedWindowLocationActions,
} from '../../common/test/mockWindowLocation';
...
describe(`cookieConsentController.ts`, () => {
  ...
  describe('createConsentController', () => {
    ...
    describe('save', () => {
      ...
      it('the domain of the cookie is set to <domain>.<suffix> so it is readable from *.hel.fi and *.hel.ninja', () => {
        ...
        controller.save();
        expect(
          mockCookieHelpers.getSetCookieArguments().options.domain
        ).toEqual('hel.fi');
  ...
  describe('createStorage', () => {
    ...
    it('Updates given consent and returns a new copy of data without mutating old data', () => {
      ...
      const approvedVersion = storage.approve([
        'requiredConsent1',
        'optionalConsent1',
      ]);

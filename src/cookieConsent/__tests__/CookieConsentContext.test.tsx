...
describe('CookieConsentContext ', () => {
    ...
    return (
      <div>
        {allUserConsentsAreHandled && (
          <span data-testid={`${consumerId}-all-handled`}></span>
        )}
        {!allUserConsentsAreHandled && (
          <span data-testid={`${consumerId}-all-not-handled`}></span>
        )}
        {willRenderCookieConsentDialog && (
          <span data-testid={`${consumerId}-should-render`}></span>
        )}
        {!willRenderCookieConsentDialog && (
          <span data-testid={`${consumerId}-should-not-render`}></span>
        )}
        <button
          data-testid={`${consumerId}-approve-all-button`}
  ...
  describe('willRenderCookieConsentDialog ', () => {
    it('is false all consents are true/false', () => {
      verifyConsumersShowConsentsNotHandled(
  ...
  describe('onConsentsParsed is called when context is created and controller has read the cookie. ', () => {
    it('Arguments are ({consents}, false) when user has not handled all consents', () => {
      ...
      expect(onConsentsParsed).toHaveBeenLastCalledWith(
        allNotApprovedConsentData.cookie,
        false
      );
    });
  ...
  describe('onAllConsentsGiven ', () => {
    it('is called only when user has given all consents.', () => {
      ...
      expect(onAllConsentsGiven).toHaveBeenLastCalledWith(
  ...
  describe('Saving ', () => {
    it('by clicking "Approve all" sends also unknown consents', () => {
      ...
      ).toEqual({ ...allApprovedConsentData.cookie, ...unknownConsents });
    });
  });
});

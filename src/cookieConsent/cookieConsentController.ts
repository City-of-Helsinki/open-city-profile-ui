...
function createCookieController(): {
  ...
  const get = (): string =>
    cookieController.get(COOKIE_NAME, { doNotParse: true }) || '';
...
export function createStorage(
  ...
  const findConsentSource = (
    consentName: string,
    targetStorage: ConsentStorage
  ): ConsentObject | undefined => {
    if (!_.isUndefined(targetStorage.required[consentName])) {
      return targetStorage.required;
    }
    if (!_.isUndefined(targetStorage.optional[consentName])) {
...
function verifyConsentProps({
  optionalConsents,
  requiredConsents,
}: ConsentControllerProps) {
  ...
  requiredConsents.forEach(consent => {
    if (optionalConsents.includes(consent)) {
      throw new Error(
        `optional consent '${consent}' found in requiredConsents.`
      );
    }
  });
}

export default function createConsentController(
  props: ConsentControllerProps
): ConsentController {
  ...
  const required = mergeConsents(
    ...
    _.pick(currentConsentsInCookie, requiredConsents)
  );

  const optional = mergeConsents(
    convertStringArrayToKeyConsentObject(optionalConsents),
    _.pick(currentConsentsInCookie, optionalConsents)
  );

  const unknownConsentKeys = Object.keys(currentConsentsInCookie).filter(
    key => !allConsents.includes(key)
  );

  const unknown = unknownConsentKeys.length
    ? _.pick(currentConsentsInCookie, unknownConsentKeys)
    : undefined;
  ...
  return {
    ...
    getRequiredWithoutConsent: () =>
      requiredConsents.filter(key => storage.getConsentByName(key) !== true),
    rejectAll,
    getUnhandledConsents: () => {
      const storedCookies = parseConsents(cookieController.get());
      return allConsents.filter(key => _.isUndefined(storedCookies[key]));
    },
    save,
  };
}

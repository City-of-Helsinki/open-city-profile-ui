...
describe('countryCallingCode.utils', () => {
  ...
  const alphaBeticallyFirstCountryInMultipleHits = countriesWithMatchingCallingCodesInAlphabeticalOrder[0];
  const alphaBeticallyFirst = {
    countryAlpha2Code: alphaBeticallyFirstCountryInMultipleHits,
    countryCallingCodes: getCallingCodesForCountryCode(countryCallingCodes, alphaBeticallyFirstCountryInMultipleHits),
  };
  describe('getCountryAndCallingCodeForNumber ', () => {
    ...
    it('returns countryCallingCode and countryAlpha2Code when match is found from multiple countryCallingCodes', () => {
      [...countriesWithMultipleCallingCodes, ...countriesWithSingleCallingCodes].forEach((countryAlpha2Code) => {
        const callingCodesForCountry = getCallingCodesForCountryCode(countryCallingCodes, countryAlpha2Code);
        callingCodesForCountry.forEach((countryCallingCode) => {
          expect(getCountryAndCallingCodeForNumber(`${countryCallingCode}1234567`)).toEqual({
            countryCallingCode,
            countryAlpha2Code,
          });
        });
    ...
    it('if multiple countries have the same country calling code, return the first alphabetically', () => {
      countriesWithMatchingCallingCodesInAlphabeticalOrder.forEach((countryAlpha2Code) => {
        const callingCodesForCountry = getCallingCodesForCountryCode(countryCallingCodes, countryAlpha2Code);
        callingCodesForCountry.forEach((countryCallingCode) => {
          expect(getCountryAndCallingCodeForNumber(`${countryCallingCode}1234567`)).toEqual({
    ...
    it(`if multiple countries have partially same country calling code,
      ...
      countriesWithPartialCallingCodesMatches.forEach((countryAlpha2Code) => {
        const callingCodesForCountry = getCallingCodesForCountryCode(countryCallingCodes, countryAlpha2Code);
        const resultDependsOnAlphabeticalOrder =
          countriesWithMatchingCallingCodesInAlphabeticalOrder.includes(countryAlpha2Code);
        ...
        callingCodesForCountry.forEach((countryCallingCode) => {
          ...
          expect(getCountryAndCallingCodeForNumber(`${countryCallingCode}1234567`)).toEqual(expectedResult);
        });
      });
    });
  });
  describe('splitNumberAndCountryCallingCode returns an object with countryCallingCode and number and ', () => {
    const testNumbers = ['', '5012345678', 'BE_can_have_chars', ' +358 _ +++', '+00000'];
    ...
    it('returns countryCallingCode and number without the country calling code when match is found', () => {
      [...countriesWithMultipleCallingCodes, ...countriesWithSingleCallingCodes].forEach((countryAlpha2Code) => {
        ...
        callingCodesForCountry.forEach((countryCallingCode) => {
          testNumbers.forEach((number) => {
            expect(splitNumberAndCountryCallingCode(`${countryCallingCode}${number}`)).toEqual({
              countryCallingCode,
              number,
            });
          });
  ...
  describe('getCallingCodesForCountryCode ', () => {
    it('creates a list with calling codes and country name', () => {
      ...
      [...countriesWithSingleCallingCodes, ...countriesWithMultipleCallingCodes].forEach((countryAlpha2Code) => {
        const callingCodesForCountry = getCallingCodesForCountryCode(countryCallingCodes, countryAlpha2Code);
        callingCodesForCountry.forEach((countryCallingCode) => {
          const option = list.find((item) => item.value === countryCallingCode);
          const label = option?.label;
          expect(label?.includes(countries.getName(countryAlpha2Code, 'en') as string)).toBeTruthy();

...
import { GdprServiceConnectionService, GdprServiceConnectionsRoot } from '../../graphql/typings';
...
export const getServiceConnectionsResultAndError = (queueController: QueueController) =>
  getActionResultAndErrorMessage<GdprServiceConnectionService[]>(serviceConnectionsType, queueController);
...
const serviceConnectionsQueryExecutor: ActionExecutor = async (action) => {
  ...
  if (!result || !result.data) {
    return Promise.reject(new Error("'No results in GDPR_SERVICE_CONNECTIONS query"));
  }
  return Promise.resolve(getServiceConnectionsServices(result.data, getData(action, 'serviceName') as string));
};
...
export function createActionForGettingSpecificServiceConnection(serviceName: string): ActionProps {
  return {
    type: serviceConnectionsType,
    executor: serviceConnectionsQueryExecutor,
    options: {

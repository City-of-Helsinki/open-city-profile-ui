...
export function getDeleteMyProfileMutationResult(errorCodes?: string[]) {
  const mockServices = getServiceConnectionsServices<Service>(getMyProfileWithServiceConnections());
  const results: GdprDeleteMyProfileMutationDeleteMyProfileResultsFragment[] = mockServices.map((service, index) => {
    const errors =

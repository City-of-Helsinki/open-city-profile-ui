...
export type ErrorRenderer = ({ message, options }: { message: string; options?: Record<string, unknown> }) => ReactNode;
...
export function getError<FormValues>(
  ...
  if (getIsInvalid(formikProps, fieldName, isOldData) && typeof errorMessage === 'string') {
...
export function getFieldError<FormValues>(
  ...
  const renderError: ErrorRenderer = ({ message, options }) => t(message, options);
...
export function createFormFieldHelpers<FormikValues>(
  ...
  hasFieldError: (formikProps: FormikProps<FormikValues>, type: keyof FormikValues) => boolean;
  getFieldErrorMessage: (formikProps: FormikProps<FormikValues>, type: keyof FormikValues) => ReactNode | undefined;
} {
  return {
    hasFieldError: (formikProps, type) => getIsInvalid<FormikValues>(formikProps, type as string, !isNew),
    getFieldErrorMessage: (formikProps, type) => getFieldError<FormikValues>(t, formikProps, type as string, !isNew),
  };
}

...
describe('parseDeleteProfileResult', () => {
  it('returns an object with failed and successful service connection deletions', () => {
    ...
    const resultWithNoFailures = parseDeleteProfileResult(
      getDeleteMyProfileMutationResult()
    );
  ...
  it('If result.success is not true, the service is in "failures"-array', () => {
    const resultWithOneFailure = parseDeleteProfileResult(
      getDeleteMyProfileMutationResult(['', 'error'])
    );
    expect(resultWithOneFailure.failures).toHaveLength(1);
    expect(resultWithOneFailure.successful).toHaveLength(1);
    const resultWithTwoFailures = parseDeleteProfileResult(
      getDeleteMyProfileMutationResult(['error', 'error'])
    );
  ...
  it('If result or result.service does not exist, the result is in "failures"-array', () => {
    ...
    const results = (
      profileResult.deleteMyProfile as GdprDeleteMyProfileMutationDeleteMyProfileFragment
    ).results;

...
it('returns correct array of phones', () => {
  ...
  expect(phonesPickedByDefault).toEqual([profile.myProfile?.phones?.edges[1]?.node]);
  const phonesWithKeptPrimary = getPhonesFromNode(getMyProfile(), true);
  expect(phonesWithKeptPrimary).toHaveLength(2);
  expect(phonesWithKeptPrimary).toEqual(profile.myProfile?.phones?.edges.map((edge) => edge?.node));
});

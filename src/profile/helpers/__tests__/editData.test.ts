...
describe('editData.ts ', () => {
  ...
  const createUniqueEditDataValue = (
    ...
    if (dataType === 'emails') {
      ...
      (newValue as Mutable<BasicDataValue>).firstName =
        createNewValue('firstName');
      (newValue as Mutable<BasicDataValue>).lastName =
        createNewValue('lastName');
    } else if (dataType === additionalInformationType) {
      (newValue as Mutable<AdditionalInformationValue>).language =
        createNewValue('language') as Language;
    }
    return newValue;
  };
  ...
  const createTestDataWithNewItem = (dataType: AddableDataType) => {
    const profileDataWithoutItems = {
      myProfile: getProfileDataWithoutSomeNodes({
        dataType,
        profileData: myProfile.myProfile as ProfileData,
        noNodes: true,
      }),
    } as ProfileRoot;
    const editor = createEditorForDataType(profileDataWithoutItems, dataType);
    ...
    return {
      ...
      newList,
      newItem,
      profileData: profileDataWithoutItems.myProfile as ProfileData,
    };
  };
  ...
  allDataTypes.forEach((dataType) => {
    ...
    when dataType is ${dataType} and returns formValues`, () => {
      const { getEditData, updateItemAndCreateSaveData } =
    ...
    it(`updateData updates correct items when dataType is ${dataType}`, () => {
      const { getEditData, updateItemAndCreateSaveData, updateData } =
        createEditorForDataType(myProfile, dataType);
      const editDataList = getEditData();
      editDataList.forEach((target, index) => {
        const newValue = createUniqueEditDataValue(target, dataType);
  ...
  addableDataTypes.forEach((dataType) => {
    describe(`addItem `, () => {
      ...
      it(`creates new ${dataType} item and helper functions detect it correctly`, () => {
        ...
        expect(pickItemIds(newList)).toEqual(['']);

        expect(isNewItem(newItem)).toEqual(true);
        expect(isNewItem(newList[0])).toEqual(true);
    ...
    describe(`movePrimaryAsFirst `, () => {
      ...
      const moveAndVerify = (
        ...
        const currentPrimary = nonArrangedList.filter(
          (item) => item.primary
        )[0];
  ...
  describe(`getEmailEditDataForUI() `, () => {
    ...
    it(`returns the primary email picked from email items. The primaryEmail prop is not used.`, () => {
      const profileQueryWithoutPrimaryEmailProperty =
        getMyProfileQueryWithoutSomeNodes({
          clearPrimary: true,
          dataType,
        });
    ...
    it(`When a new email is added, it has 'primary' set to 'true' and all emails are included in the save data`, () => {
      const profileQueryWithoutPrimaryEmails =
        getMyProfileQueryWithoutSomeNodes({ noPrimary: true, dataType });
      const { getEditData, addItem, updateItemAndCreateSaveData } =
      ...
      const newEmailInSaveData = (
        saveData.emails as EmailNode[]
      ).pop() as EmailNode;
